...
class ArduinoArmHardware(SystemInterface):
    ...
    def on_init(self, hardware_info: SystemInterface.HardwareInfo) -> CallbackReturn:
        ...
        """
        # Initialize rclpy if not already initialized by the lifecycle manager
        # This is a fallback and might not be strictly necessary if controller_manager handles it
        if not rclpy.ok():
        ...
        self._serial_port = hardware_info.hardware_parameters.get('serial_port')
        ...
        if not self._serial_port:
            self._node.get_logger().error("Serial port parameter 'serial_port' not found in URDF.")
            return CallbackReturn.ERROR
        if self._num_joints <= 0:
            self._node.get_logger().error(f"Invalid number of joints specified: {self._num_joints}. Must be greater than 0.")
            return CallbackReturn.ERROR
    ...
    def on_configure(self, previous_state: rclpy.lifecycle.State) -> CallbackReturn:
        ...
        try:
            ...
            self._node.get_logger().error(f"An unexpected error occurred during configuration: {e}")
            return CallbackReturn.ERROR
    ...
    def on_cleanup(self, previous_state: rclpy.lifecycle.State) -> CallbackReturn:
        ...
        # Destroy the node if it was created by this class
        if self._node:
    ...
    def on_shutdown(self, previous_state: rclpy.lifecycle.State) -> CallbackReturn:
        ...
        # rclpy.shutdown() # Only shutdown if rclpy.init was called without args, and only once
    ...
    def export_state_interfaces(self):
        ...
        """
        state_interfaces = []
        for i, joint_info in enumerate(self.info.joints): # Use joint_info to access name
            state_interfaces.append(SystemStateHandle(joint_info.name, "position", self._hw_states[i]))
    ...
    def export_command_interfaces(self):
        ...
        command_interfaces = []
        for i, joint_info in enumerate(self.info.joints): # Use joint_info to access name
            command_interfaces.append(SystemCommandHandle(joint_info.name, "position", self._hw_commands[i]))
        ...
        with self._serial_lock:
            ...
            for i in range(self._num_joints):
                ...
                if i < len(self._latest_arduino_steps):
                    # Added a check for steps_per_unit to prevent division by zero
                    if self._steps_per_unit[i] != 0:
                        self._hw_states[i] = self._latest_arduino_steps[i] / self._steps_per_unit[i]
                    else:
                        self._node.get_logger().warn(f"steps_per_unit for joint {i} is zero. Cannot convert steps to position.")
                        self._hw_states[i] = 0.0 # Set to default or error state
                else:
        ...
        with self._serial_lock:
            ...
            for i in range(self._num_joints):
                ...
                if self._steps_per_unit[i] != 0:
                    ...
                    self._node.get_logger().warn(f"steps_per_unit for joint {i} is zero. Cannot convert command to steps.")
                    target_steps = 0 # Default to 0 steps if conversion is impossible
